...
sample_info <- "~/Documents/GCproject_final/data/bams/all_sample_info2.csv"
...
ggsave(plot = fp,
       filename = paste0(output, "/graphs2/", i, ".fp.png"),
       device = "png",
...
combine_fps <- function(sample.ids, mean.colour, output, group.name, scale = FALSE){
  ...
  if(scale == FALSE){
    saveRDS(combined.counts, paste0(output, "/counts3/", group.name, ".df.RDS"))
  } else {
    saveRDS(combined.counts, paste0(output, "/counts3/", group.name, ".df.scaled.RDS"))
  }
  ...
  if(scale == FALSE){
    ...
    ggsave(filename = paste0(output, "/graphs4/", group.name, ".unscaled.fp.png"),
           device = "png",
           width = 12,
           height = 1.5,
           units = "in")
    ...
    ggsave(filename = paste0(output, "/graphs4/", group.name, ".scaled.fp.png"),
           device = "png",
           width = 12,
           height = 1.5,
           units = "in")
